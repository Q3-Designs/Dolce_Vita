...
const Navbar:React.FC<NavbarProps> = ({links,id}) => {
...
backgroundImage: isMobile ? 'linear-gradient(to right, #00bfff, #4db8ff, #64bcff, #00bfff)' : 'none',
transition: isMobile? 'height 0.3s ease-in,padding 0.3s ease-in' : 'none',
...
paddingTop:subMenuVisible ?'1rem' : '0'
  ...
  const navStyle = (index:number|null) => {

    return{
      backgroundColor: navHovered === index? '#00bfff' : 'transparent',
