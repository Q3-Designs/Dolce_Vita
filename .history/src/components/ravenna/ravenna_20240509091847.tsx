...
const outfrontImages = Array.from({ length: 6 }, (_, index) => ({
    url: `ravenna${index + 2}`,
    alt: `ravenna${index + 2}`
}));

const backyardImages = Array.from({ length: 7 }, (_, index) => ({
    url: `ravenna${index + 8}`,
    alt: `ravenna${index + 8}`
}));

const indoorsImages = Array.from({ length: 8 }, (_, index) => ({
    url: `ravenna${index + 15}`,
    alt: `ravenna${index + 15}`
}));

const bedAndBathImages = Array.from({ length: 7 }, (_, index) => ({
    url: `ravenna${index + 23}`,
    alt: `ravenna${index + 23}`
}));

console.log('out front images',outfrontImages)
